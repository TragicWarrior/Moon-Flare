...
int main(void)
{
    ...
    if (n != 2) { printf("FAIL: expected n=2, got %zu\n", n); return 1; }
    if (ops[0].abi != MF_PLUGIN_ABI) { printf("FAIL: battery abi\n"); return 1; }
    if (ops[0].ops_size != sizeof(mf_plugin_ops_t)) { printf("FAIL: battery ops_size\n"); return 1; }
    if (strcmp(ops[0].kind, "battery") != 0 || strcmp(ops[0].driver, "demo") != 0) {
        printf("FAIL: battery kind/driver\n"); return 1;
    }
    if (ops[1].abi != MF_PLUGIN_ABI) { printf("FAIL: charger abi\n"); return 1; }
    if (ops[1].ops_size != sizeof(mf_plugin_ops_t)) { printf("FAIL: charger ops_size\n"); return 1; }
    ...
    if (!bat_a || !bat_b || !chg) { printf("FAIL: open\n"); return 1; }
    ...
    /* fd() */
    if (ops[0].fd(bat_a) != -1) { printf("FAIL: bat_a fd\n"); return 1; }
    if (ops[0].fd(bat_b) != -1) { printf("FAIL: bat_b fd\n"); return 1; }
    if (ops[1].fd(chg) != -1)   { printf("FAIL: chg fd\n");   return 1; }
    printf("OK: fd() == -1\n");

    /* select_mask */
    if (ops[0].select_mask(bat_a) != 0) { printf("FAIL: select_mask\n"); return 1; }
    printf("OK: select_mask == 0\n");

    /* caps */
    unsigned ca = ops[0].caps(bat_a);
    if ((ca & MF_CAP_READ) == 0) { printf("FAIL: no READ cap\n"); return 1; }
    if ((ca & MF_CAP_ACTION_SWITCH) == 0) { printf("FAIL: no ACTION_SWITCH cap\n"); return 1; }
    printf("OK: caps READ|ACTION_SWITCH\n");

    /* get_reading battery — check cell 1 seed offset */
    char buf_a[2048], buf_b[2048];
    if (ops[0].get_reading(bat_a, buf_a, sizeof(buf_a)) < 0) { printf("FAIL: reading a\n"); return 1; }
    if (ops[0].get_reading(bat_b, buf_b, sizeof(buf_b)) < 0) { printf("FAIL: reading b\n"); return 1; }

    double va = parse_first_cell_v(buf_a);
    double vb = parse_first_cell_v(buf_b);
    if (va < 0 || vb < 0) { printf("FAIL: parse cell v\n"); return 1; }
    if (fabs(va - vb) < 0.0001) { printf("FAIL: batteries not isolated (cell1 same)\n"); return 1; }
    ...
    char buf_c[512];
    if (ops[1].get_reading(chg, buf_c, sizeof(buf_c)) < 0) { printf("FAIL: charger reading\n"); return 1; }
    const char *stage = parse_field(buf_c, "charge_stage");
    if (!stage) { printf("FAIL: no charge_stage\n"); return 1; }
    int found = 0;
    for (int i = 0; valid_stages[i]; i++)
        if (strcmp(stage, valid_stages[i]) == 0) { found = 1; break; }
    if (!found) { printf("FAIL: invalid charge_stage '%s'\n", stage); return 1; }
    ...
    if (ret_a != MF_OK) { printf("FAIL: bat_a set_switch=%d err='%s'\n", ret_a, err); return 1; }
    ...
    if (fabs(va2 - va) > 0.0001) { printf("FAIL: bat_a changed after action (not isolated)\n"); return 1; }
    ...
    if (ret_c == MF_OK) { printf("FAIL: charger should not support set_switch\n"); return 1; }
