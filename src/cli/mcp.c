...
static cJSON *build_tools_list(void)
{
    ...
    cJSON_AddItemToArray(tools, make_tool("status",
        ...
        "actuators, services; a weather service carries data.weather, an SMS notification "
        "service data.channel and data.credits_remaining), plus a \"system\" object with "
        "totals over the active, online modules: charger input watts, capacity-weighted "
        "SOC, stored/capacity Wh, battery charge/discharge watts, and what the packs can "
        "deliver and the inverters are rated for (battery_limit_w, inverter_rated_w; null "
        "unless every counted one reports it). Modules marked "
